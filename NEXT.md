...
### SD 완충·LoRa 실시간 함대 전송 (2026-09-16, 아직 보드에 안 올림)
...
- 안전상 부팅 기본은 수신 전용이다. `boat 1~32`를 지정하고 `lora live on`을 직접 실행해야 위치 방송이 시작되며,
  재부팅하면 다시 꺼진다. `lora peers`는 최근 3초에 받은 배와 RSSI/SNR을 보여준다.
- 호스트 회귀시험 162개와 IDF 전체 빌드는 통과했다. **보드 플래시, 실외 PPS, 두 보드 송수신은 아직이다.**
...
1. 새 IDF 판을 보드에 올린 뒤 부팅 줄에서 `버퍼 1024 KB`를 확인하고, 실외에서 `lora`의 PPS 나이가 1초마다 갱신되는지 본다.
