...
**`flags` 의 bit3 이 중요하다.** 이 비트가 서 있으면 `sog` 와 `cog` 는 GPS 가
아니라 시뮬레이터가 지어낸 값이다. 위성을 못 잡았을 때 값을 비워 보낼 방법이
없어서 그렇게 채운다. **수신 측은 이 비트를 반드시 확인하고 사용자에게
표시해야 한다.** 지어낸 숫자를 실측으로 오해하면 바다에서 위험하다.

`cog`(GPS 침로)와 `hdg`(자력계 방위)는 다른 값이다. 앞은 배가 실제로 가는
방향이고 뒤는 뱃머리가 보는 방향이다. 조류와 바람 때문에 벌어지며, 배가
멈춰 있으면 `cog` 는 의미가 없고 `hdg` 만 쓸 수 있다.

> 37바이트를 보내려면 MTU 협상이 필요하다. 실측에서 iOS 는 255 로 협상했고,
