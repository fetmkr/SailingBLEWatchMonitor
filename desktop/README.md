...
```
마우스를 옮기면    옅은 선이 따라온다. 영상도 그 자리로 훑어진다
누르면            그 자리에 박힌다 (파란 선 + 위쪽 삼각형)
끌면              고정은 그대로 두고 화면이 밀린다
Esc               고정을 푼다
```

**박아 두지 않으면 화면을 볼 수가 없다.** 마우스를 뗄 때마다 풀리면 영상을
보려고 눈을 옮기는 순간 자리를 잃는다. 그래서 누르면 고정된다.

고정한 뒤에는 마우스를 움직여도 영상이 안 따라간다. 박아 둔 자리를 보려고
고정한 것이다. 자리를 옮기려면 다시 누른다.

4픽셀 안에서 떼면 "눌렀다", 그보다 움직였으면 "끌었다" 로 본다.

## 영상 슬라이더

영상 칸 아래에 있다. 타임라인 없이 영상만 훑을 때 쓴다. **끌면 타임라인
고정도 같이 따라간다** — 둘이 늘 같은 곳을 봐야 한다.
