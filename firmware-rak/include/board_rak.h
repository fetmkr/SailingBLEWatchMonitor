...
//
// 이 핀이 3V3_S 를 켜고 끈다. LOW 면 센서 슬롯 A~D 에 꽂힌 모듈이 죽는다.
//     [확인: RAK19007 datasheet — "Set IO2=0, 3V3_S is off"]
//
// ★ 화면과 IMU 는 여기 안 걸린다. 둘 다 항상 켜져 있는 VDD 를 쓴다.
//   (예전에 이 자리에 "화면도 GPS 도 IMU 도 한꺼번에 안 보인다" 고 적혀
//   있었는데 틀렸다. 재보고 고쳤다.)
//
//     [확인: 2026-09-04, `power off` 뒤에]
//       GPS   5초 동안 한 바이트도 안 옴          → 3V3_S 다. 꺼진다
//       화면  I2C 스캔에 0x3C 그대로 보임         → VDD 다. 안 꺼진다
//       IMU   I2C 스캔에 0x68 그대로 보임         → VDD 다. 안 꺼진다
//
//   근거도 맞물린다. 화면은 J12 헤더에 꽂는데 그 1번 핀이 VDD 다
//   [확인: RAK19007 datasheet — J12 Header Pinout, 1번 = VDD 3.3V].
//
//   ※ 보드를 재울 때 이게 중요하다. 3V3_S 만 내리면 화면이 마지막 그림을
//     켠 채로 남아 전류를 계속 먹는다. 화면과 IMU 는 따로 재워야 한다.
//
// 문서끼리 값이 달랐다.
...
// 그래서 모듈을 꽂아도 안 물린다 — 저장 버튼을 여기 단다.
//
// 실측 (2026-08-27, `pin` 명령):
//   AIN1 (GPIO2)   풀업 HIGH · 풀다운 LOW   비어 있다
//   IO1  (GPIO21)  풀업 LOW  · 풀다운 LOW   슬롯 A 의 GPS 가 잡고 있다
//   IO2  (GPIO14)  센서 전원 스위치라 못 쓴다
//
// ★ AIN0 에는 배터리 분압이 물려 있다. 버튼으로 쓰면 안 된다.
static constexpr int kAin0 = 1;   // 배터리 분압. 건드리지 말 것
static constexpr int kAin1 = 2;   // J11 1번. 저장 버튼
