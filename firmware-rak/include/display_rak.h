...
/// GPS 움직임 종류. 속도 옆에 낱말로 그린다.
    ...
    /// GPS 움직임 종류 — **늘** 채운다. 배터리 전압 옆에 늘 그린다.
    ///
    /// 위의 gnssMode 는 "정해 둔 모드와 다를 때만" 속도 줄에 경고로 뜬다.
    /// 이건 그와 따로, 지금 모듈이 **실제로 답한** 모드를 늘 보여준다.
    /// 사용자가 "배터리 전압 옆에 현재 모드 표시해. 항상." 이라고 했다
    /// (2026-09-13). 보트모드가 아닌 채로 나가서 저속을 통째로 잃은 적이
    /// 여러 번이라, 물 위에서 한눈에 확인할 수 있어야 한다.
    ///
    /// h 휴대 · s 정지 · p 보행 · c 자동차 · b 선박 · ? 모름(되물어보기 실패)
    char gnssModeNow = '?';
