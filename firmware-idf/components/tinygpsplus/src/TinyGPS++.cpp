// ── firmware-idf 에서 바꾼 곳 (원본: firmware-rak/.pio/libdeps/rak3112/TinyGPSPlus 1.1.0) ──
//   1) `#if !defined(ARDUINO)` 의 std::chrono millis() 를 esp_timer_get_time()/1000 으로 바꿨다.
//      (그 시계는 system_clock 이라 settimeofday 에 같이 뛴다 — 아래 주석)
//   2) Arduino.h 가 주던 byte · TWO_PI · DEG_TO_RAD · RAD_TO_DEG · radians · degrees · sq 를
//      이 파일 안에서 같은 정의로 둔다.
//   그 밖의 줄은 원본 그대로다.
/*
...
// [firmware-idf] 원래 여기는 std::chrono::high_resolution_clock 로 millis() 를 만들었다.
//   libstdc++ 에서 그 시계는 system_clock 이다 (c++/15.2.0/bits/chrono.h:1294). 펌웨어가 GPS 로
//   settimeofday 를 부르면 그 시계가 수십 년 뛰어서 age() 가 틀어진다. 부팅 뒤 흐른 시간(esp_timer)으로 바꿨다.
#include "esp_timer.h"
...
// [firmware-idf] Arduino.h 가 주던 것 (arduino-esp32 cores/esp32/Arduino.h 45-47, 78-80, 145 와 같은 정의)
