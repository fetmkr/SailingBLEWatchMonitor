...
**09-19 함대 지도 HDG·정지 COG·AK8963 재부팅 (A3·A6·D2, 실물 적용)**
- 스크린샷의 선수 표식은 `HDG 091°T`와 같은 91°를 가리켰다. 큰 차이로 보인 `146.1°T`는 SOG 0.00에서도 노출된 GPS COG였다.
  실측 중 정지 COG가 228°→208°→269°→304°로 돌아, 펌웨어는 정지 판정 시 COG만 무효로 보내고 앱도 예전 패킷의 SOG 0 COG를 거절한다. 함대 목록은 COG 대신 `SOG · HDG T/M`을 명시한다.
- AK8963 `CNTL1` 쓰기가 리셋 직후 `SLV0_EN=0`인 상태에서 이전 거울 설정에 기대고 있었고, bit-depth도 불안정한 거울값에서 보존했다. 재부팅마다 14/16-bit가 바뀌어 자력 크기가 약 4배 달랐다.
  쓰기마다 SLV0 1-byte transaction을 명시하고, ASA 3축을 한 번에 읽으며, `16-bit + 8 Hz`를 매번 직접 설정·표본에서 재검사한다. ASA는 재부팅 두 번 모두 `A9 AB A0`.
- 이전 v1 자력 보정 `(5.7, 16.7, -3.3) µT`은 수평 자기장을 대부분 제거해 HDG를 예민하게 만들었다. NVS를 `/tmp/sender-nvs-before-magclear.bin`(SHA-256 `21e0b3f6...a86e49`)에 보존한 뒤 이 보정만 지웠다.
  지운 뒤 같은 자세의 두 부팅 HDG는 `291.6°M`, `291.3°M`(차이 0.3°), ASA·자력 크기도 반복 일치했다. 장착각은 사용자가 확인한 180°(`-180°`로 정규화), 편각은 별도 `-9.02°` 유지. 최종 LoRa 배 2·1 Hz·PPS 준비 확인.
- 호스트 회귀 172개, 함대 통신 시험, 데스크탑 빌드, ESP-IDF v6.1 전체 빌드 통과. 새 v2 3축 보정과 Iris 50 사방향 비교는 남았다.

**09-19 LoRa HDG M·함대 실시간 그래프 (F3·F4, 실물 적용 완료)**
- 두 보드에 `web-picker-fast-136-g312ae86`을 올렸다. 무선 v2 24바이트에 HDG M을 추가하고 Fleet BLE v3를 34바이트로 확장했다.
- 앱은 지도 배 표식에 SOG·HDG를 표시하고 최근 5분 SOG, HDG/COG, 힐/피치, RSSI/SNR 그래프를 그린다.
- 송신 B02→수신 B00 실내 PPS 1 Hz에서 118패킷 이상, RSSI -77 dBm, SNR 12~14, CRC·무선 링·앱 큐 버림 0을 확인했다.
  HDG M은 SOG/COG가 품질 거절로 비어도 계속 들어왔다. 아이패드 빌드 `0.1.0.2026091902` 설치·실행 뒤
  수신 보드 BLE Fleet v3 알림 5회 이상 성공·실패 0을 확인했다. 지도·그래프의 실기 화면 판독은 사용자가 이어서 확인한다.
...
- 09-17 실물: 광고 status에 `boat_id + 1`을 실어 `수신 보드 찾기`에는 0번 `SAIL-3282`만 보이고 B07 `SAIL-random()`은 숨겨짐을 확인했다.
  앱 완전 종료→재실행과 USB 보드 리셋 두 번 모두 기억한 `SAIL-3282`로 자동 재연결했고, 10초 상태 확인이 LoRa 수신을 다시 켜는 것까지 부팅 로그로 확인했다.
- 09-17 두 보드에 `web-picker-fast-132-gb641b8f`를 같이 플래시했다. PPS가 없는 B01이 5~8초 간격 확인 신호를 22초 동안 4번 보냈고,
  0번 보드는 4번 모두 수신했다(RSSI -82 dBm, SNR 12~13, CRC·버림·송신 실패 0). 패킷은 `fix 0 · time 0 · SOG/COG 없음`이었고,
  실제 함대 앱도 `1번 배 · 보드 켜짐 · GPS 시각 대기`로 표시했다. 실외 PPS 뒤 1 Hz 위치 TDMA는 아직 남았다.
...
| ID | 요구사항 | 기능 | 코드 | 빌드 | 보드 시험 방법 | 합격 기준 | 보드 시험 | 증거 |
|---|---|---|---|---|---|---|---|---|
| L01 | F1 | 켤 때 설정 줄 · 전파시간 | ✅ | ✅ | 켤 때 · `lora` | 922.55 · SF7 · BW500 · 8 dBm · 14144 us | ✅ 09-19 두 보드 모두 24바이트 14144 us. 22→24바이트가 같은 LoRa 심볼 수 구간 | |
...
| L04 | — | 두 보드 주고받기 | ✅ | ✅ | watch/tx | 받은 수 늘고 CRC 오류 0 | ✅ 09-19 B02 PPS 1 Hz 118개 이상 수신, CRC·버림 0, RSSI -77 dBm·SNR 12~14 | |
...
| L07 | F3 | 24바이트 실시간 패킷·GPS PPS 32슬롯·20분 holdover·PPS 전 확인 신호·송수신 일꾼·off sleep | ✅ | ✅ | 워치에서 배 번호 저장 → 장거리 시작 → 실외 60초 → 종료 | 시작 전/종료 후 SX1262 sleep · PPS 전 HDG 수신 · PPS 뒤 송신 약 60 · 실패/차례 놓침 0 | 🔶 09-19 실내 GPS PPS 1 Hz와 HDG 연속 수신은 통과. 20분 holdover·종료 sleep 장시간 시험은 남음 | |
| L08 | F4 | 수신 패킷 해석·최근 3초 heard·`lora peers`·번호 tie 충돌 | ✅ | ✅ | 두 보드: 송신 boat 1 / 수신 boat 0 watch+peers | 좌표·SOG·COG·HDG 일치 · CRC/버림 0 · 3초 뒤 목록에서 사라짐 | 🔶 09-19 무선 v2/BLE v3로 위치·SOG·COG·HDG·RSSI·SNR과 BLE 알림 성공 확인. 3초 만료·번호 충돌 실물 시험은 남음 | |
