...
> 2026-09-16 사용자 추가 요청으로 HDG를 식 5(3D 자력 보정 + Fusion)로 변경했다. 이 항목은 단순 이식과 별도이며,
> 이유·구현·검증 범위는 [HEADING.md](../docs/imu/HEADING.md) 맨 위에만 유지한다. 보드 적용 전.
...
- ★★ **17:45 쯤 두 번째로 USB 가 멎었다.** `board_rec_test.py gps` 가 끝난 뒤 `imu`·`clean` 단계에 보드가 한 줄도 안 냄.
  ...
  - **다시 꽂기 실측 (19:0x, 시험판, 사용자가 뽑았다 꽂음 → 포트 한 번 20초)**: 켜짐 1번 · 앱 정상 · **재시작 되풀이 안 남**
    드라이버 전 **int_ena 0x00000004 · int_st 0 · int_raw 0x000007a8** — 앞 실측(0x5a8)보다 **bit9 BUS_RESET 걸림(raw)이 새로 섰다** (다시 꽂기가 버스 리셋을 만든다 [확인])
    그러나 BUS_RESET **켜짐은 꺼져 있어** int_st 에 안 올라왔다 → 폭주 없음. 끄기 코드가 한 일은 없다 (까기 전부터 꺼져 있었다) → **이번 결과로 고침이 먹었다고 말할 수 없다**
    남는 질문: 폭주하던 옛 판 때는 BUS_RESET 켜짐이 켜져 있었나 — 그 판은 레지스터를 안 찍어 [모름]. esptool 로 올릴 때(ROM 다운로드 모드) 켜짐이 지워졌을 수 있다 [추측]
    확정하는 시험: ⓐ 드라이버 전에 BUS_RESET 켜짐을 일부러 켜고 안 지우는 판 → 다시 꽂기 → 되풀이가 나나 · ⓑ 같은 판에 끄기·지우기 넣은 판 → 다시 꽂기 → 안 나나
  - **출처 여러 개 조사 결과 (14곳 + 실측 2)**:
    ✅ 확정 (출처 5): 폭주는 **두 조건이 같이** 있어야 난다 — ① BUS_RESET 걸림(bit9, 다시 꽂으면 선다 [실측]) ② BUS_RESET 켜짐(int_ena bit9).
       IDF 6.1 드라이버는 bit9 를 안 지우고 안 켠다 [usb_serial_jtag.c:55-154, master 도 같음] · 시작 코드는 USB 콘솔 블록을 일부러 리셋 안 한다 [esp_system/port/soc/esp32s3/clk.c:329] ·
       리셋하는 곳은 잠자기 코드뿐 [sleep_console.c:28] · 소프트웨어 리셋에 USB 상태가 남는 사례 [esp-idf #9826]
       폭주 때 코어덤프 레지스터 A7=0x200 (int_st 를 읽는 명령 자리) → 폭주하던 비트는 bit9 로 보인다 [추측 — 디스어셈블로 끌어냄]
    ❓ 미확정: **누가 bit9 켜짐을 켰나.** IDF 전체·우리 코드 0곳. 켜는 곳은 아두이노 HWCDC.cpp:343-345 뿐 (찬성) ↔ esptool 로 올린 직후 0x04 로 꺼져 있었다 (반대, esptool 스텁·ROM 이 지우는지 소스로 확인 못 함)
       판정: 아두이노 남은 켜짐 가설 **찬성 2 · 반대 2 · 무관 3**
    17:00 (0단계, 드라이버 없음 → 처리기 없음) · 17:45 (0바이트) 멎음은 **다른 현상일 가능성이 크다** — 이 폭주는 매번 부트 로그가 왔다
    확인 한 번: firmware-rak 을 다시 올리고 → 시험판 올리고 → int_ena 에 bit9 가 남나 (남으면 아두이노 남음 확정 + esptool 이 안 지운다는 것도 확정)
    **고침은 어느 설명이든 두 조건을 다 없앤다**: 드라이버 전에 드라이버가 안 다루는 켜짐을 끄고 BUS_RESET 걸림을 지운다 (저장소 app_main 에 들어 있음)
  - **확정 시험 (사용자 "진행해", 19:2x)**: firmware-rak (`pio run -t upload`, 켠 채 8초) → 고침 없는 판 (`-DSAIL_USB_NOFIX`, 바이트로 판 구분 확인) 을 esptool 로 올림 → 포트 한 번 20초
    **드라이버 전 int_ena 0x00000204** (bit9 BUS_RESET 켜짐 + bit2) · int_st 0 · int_raw 0x5a8 · 켜짐 1번 정상
    → ✅ **아두이노 HWCDC 가 켠 BUS_RESET 켜짐이 IDF 판으로 바꿔도 남는다 [확인]** · ✅ **esptool 로 올리기가 그 켜짐을 안 지운다 [확인]**
    아직 폭주가 없는 이유: BUS_RESET 걸림(raw bit9)이 안 섰다. 다음: 사용자가 다시 꽂기 → 되풀이 재시작이 나면 원인 확정 → 고침 판을 올려 첫 켜짐에 켜짐·걸림이 있는데도 안 죽는지
  - **다시 꽂기 → 재현 (19:3x, 고침 없는 판)**: 20초에 **4번 켜짐**, 매번 드라이버 전 **int_ena 0x204 · int_st 0x200** (BUS_RESET 켜짐 + 걸림) → 드라이버 깔자마자 `Interrupt wdt timeout on CPU0`
    → ✅✅ **원인 확정: 아두이노 HWCDC 가 켠 BUS_RESET 켜짐이 남은 채로 다시 꽂아 버스 리셋이 걸리면, IDF 6.1 드라이버가 그 걸림을 안 지워 처리기가 끝없이 다시 불린다**
  - **고친 판을 되풀이 중인 보드에 올림 (19:4x)**: 켜짐 1번 · 앱 정상. 그러나 첫 켜짐 드라이버 전 **int_ena 0x00000004 · int_st 0 · int_raw 0x508** — 켜짐도 걸림도 **이미 없었다**
    → ❌ **고침이 먹었다는 증거가 아니다** (끌 것이 없었다). 올리기 전후로 bit9 켜짐이 꺼졌다 — 앞 시험(firmware-rak → 올리기)에서는 안 꺼졌는데 이번에는 꺼졌다. 되풀이 중이던 보드에 올린 것이 다른 점. 왜인지 [모름]
    고침 확정 방법: firmware-rak 올리기 → **고친 판** 올리기 (드라이버 전 0x204 확인) → 다시 꽂기 → 찍힌 값이 0x204·0x200 인데 안 죽나
  - **그 방법대로 해 봄 (19:5x)**: firmware-rak → 고친 판 → 포트 한 번: 드라이버 전 **0x00000004** — 이번에도 켜짐이 이미 꺼져 있었다
    ★ 해석: `idf.py flash` 는 끝에 보드를 리셋해 **첫 켜짐이 내가 포트를 열기 전에 지나간다**. 고친 판은 그 첫 켜짐에서 켜짐을 끈다. 내가 여는 순간 리셋되는 **두 번째 켜짐**만 보이니 늘 0x4 다.
      고침 없는 판은 첫 켜짐에서 안 끄니 두 번째 켜짐에도 0x204 가 보였다. → 앞의 "왜 꺼졌나 [모름]" 도 이것으로 설명된다 [추측 — 아래 시험으로 확인]
    ★ 고친 증명 순서: firmware-rak → 고침 없는 판(0x204 확인) → 다시 꽂기(되풀이) → 고친 판을 `--after no-reset` 으로 올림 → DTR·RTS 내리고 포트를 열어 **그 리셋이 첫 켜짐**이 되게 → 드라이버 전 0x204·0x200 이 찍히고도 안 죽나
  - **그 순서대로 함 (20:0x, 사용자 다시 꽂기 — "이걸 몇번하냐고" 이번이 마지막으로 약속)**:
    ① 고침 없는 판 다시 꽂은 뒤 12초: 켜짐 3번 · Guru 2번 · 드라이버 전 int_ena 0x204 · int_st 0x200 → **재현 두 번째** ✅
    ② 고친 판 `esptool … --after no-reset write-flash @flash_args` → "Staying in bootloader"
    ③ 포트 한 번 = 고친 판 첫 켜짐: 켜짐 1번 · Guru 0 · 드라이버 전 **int_ena 0x204 (켜짐 남음 — 첫 켜짐을 본 것 확인) · int_st 0x000 · int_raw 0x528 (걸림 bit9 없음)**
    → ❌ **또 증명 안 됨.** 켜짐은 있었지만 걸림이 올리기 중에 지워졌다 (ROM 다운로드 모드가 버스 리셋 걸림을 지운 것으로 보임 [추측]). 걸림이 없으면 고침 없는 판도 안 죽는다
    → 앞의 "0x4 수수께끼" 해석(고친 판이 첫 켜짐에 끔)은 맞았다 ✅: 첫 켜짐을 보면 0x204 가 보인다
  - **고침이 먹는 근거 (보드 증명 말고)**: int_st 는 int_raw 와 int_ena 를 AND 한 값이다 (레지스터 이름·드라이버가 int_st 로 걸림을 읽음 [usb_serial_jtag_ll.h:71-74]). 드라이버 전에 bit9 켜짐을 끄면
    걸림이 서도 int_st bit9 는 0 이라 처리기가 불리지 않는다 — 논리로는 막힌다. **보드에서 켜짐+걸림이 같이 있는 순간 고친 판이 산 것을 본 적은 없다**
  - ~~앞으로 평소 다시 꽂을 때 0x204·0x200 이 찍히면 증명~~ ← **틀림.** 고친 판은 첫 켜짐에 남은 bit9 켜짐을 꺼 버린다. 그 뒤로는 아두이노 판을 다시 올리지 않는 한
    켜짐이 다시 켜질 일이 없다 (IDF 전체·우리 코드에서 bit9 를 켜는 곳 0) → 평소 꽂기로는 증명이 안 생긴다
  - 보드 증명이 필요해지면: 켜짐과 걸림이 **같이** 있는 순간에 고친 판이 켜져야 한다. 올리기(ROM) 가 걸림을 지우므로 "되풀이 중인 보드에 고친 판 올리기" 로는 안 된다.
    가능한 길 [추측]: 고침 없는 판에 "시리얼 명령으로 고침을 켜는" 스위치를 두고, 되풀이가 시작되기 전(다시 꽂은 직후 드라이버 전)에 판단하게 — 사용자 다시 꽂기가 또 필요하다. 사용자가 원할 때만
...
# 09-15 실외 세션 116 뒤 표시 정책 변경

사용자 "화면에 숨기지마"를 반영한다 (A3·A6·D2·D7).
SOG는 기존 평활 경로가 품질 때문에 거절하면 현재 RMC 원본을 표시하고 OLED에 `?`를 붙인다.
OLED COG는 SOG 유효성과 분리한다. HDG는 동일한 기울기 보정 계산을 운동 가속도에서도 제공하고
OLED에 `?`를 붙인다. SD·BLE·TXT도 같은 HDG 진입점을 쓴다. 가속도·자력 입력 자체가 없으면 값을 만들지 않는다.

HLG의 기존 방위 정책 2를 바꾸지 않고 새 정책 3을 기록한다. 앱은 둘을 구분해 재계산한다.
공유 헤더의 새 인자는 기본값이 옛 정책이라 firmware-rak의 호출 동작은 바뀌지 않는다.
장착 오프셋 +155.7°, 편각 0°, 자력 보정값은 변경하지 않았다. 이 작업은 나침반 정확도 보정이 아니다.

빌드·호스트 검증 결과는 CHECKLIST.md를 따른다. 세션 116 원본 분석 및 IMU·NAV 시각 정렬 문제는
`/Users/hojunsong/Documents/ChatGPT/sailing analyzer/field-test-20260915/항해시험-분석.md`에 있다.


## 2026-09-15 22:49 OLED 수정 보드 적용

사용자의 명시적 요청으로 `magcal-1` (소스 35bc87f, OLED 변경 397856c 포함)을 앱 영역에만 적용했다.
기존 설정은 전후 NVS 전체 비교로 보존 확인. 부팅 완료, 세션 117 정상 종료·NAV 152·IMU 1,485·누락 0·HDG 정책 3.
기록 후 파일을 회수해 SHA-256·헤더 개수·CRC를 확인했다. 실내 GPS fix 없음으로 실외 표시와 나침반 정확도는 미확인.
워치앱은 22:33 설치·22:34 실행된 판. 아이폰 앱 설치는 아직 안 했다. 자세한 시험 범위와 근거는 CHECKLIST.md 22:49 항목.
