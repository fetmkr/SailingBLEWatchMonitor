...
static void updateLoraLive(uint32_t now) {
    ...
    // 지도와 GPS COG는 진북(T) 기준이다. LoRa v3에만 편각을 더한 HDG T를 싣는다.
    // OLED·워치·HLG의 기본 HDG M은 그대로 두며, PPS 전 확인 신호에도 방향은 보낸다.
    const float heading = boatHeadingDeg();
    v.headingValid = std::isfinite(heading) && heading >= 0.0f;
    if (v.headingValid) v.headingDeg = hdg::wrap360(heading + gHdgDeclDeg);
...
static void controlLine(const char* raw) {
    ...
    if (!strcmp(line, "rec on") || !strcmp(line, "rec start")) {
        const bool ok = recWantOn("BLE rec on");
        if (ok) {
            ble::controlSay("ok rec on");
        } else {
    ...
    if (!strcmp(line, "rec off") || !strcmp(line, "rec stop")) {
        recWantOff("BLE rec off");
        ble::controlSay("ok rec off");
        return;
    }
    if (!strcmp(line, "rec")) {
